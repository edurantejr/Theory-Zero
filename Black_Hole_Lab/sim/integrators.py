...
def step(state: dict[str, xp.ndarray],
         ...
         dx: float) -> xp.ndarray:
    """
    1) Evolve the metric: g → g + dt * (R00(S) - damping * g)
    2) Compute ∂R00/∂x = (Fx, Fy, Fz)
    3) Gather F at each particle’s lattice cell
    4) Kick velocities and drift positions
    ...
    # —————————————————————————————————
    # 2) Make sure particles live on the same backend
    particles = as_backend(particles)

    # —————————————————————————————————
    # 3) Compute force field ∂R00/∂x
    Fx, Fy, Fz = force_phase3(g_new, dx)

    # —————————————————————————————————
    # 4) Convert continuous positions → integer lattice indices
